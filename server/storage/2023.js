const an2023 = {
  an: 2023,
  subiecte: [
    {
      tip: "bac", //ID-ul din URL
      nume: "Examenul național de bacalaureat 2023",
      info: "Toate subiectele sunt obligatorii. Se acordă zece puncte din oficiu.",
      puncte: [
        //SUB I
        {
          cerinta:
            "Harta de mai sus se referă la subiectul I A – E. Pe hartă sunt marcate state cu litere şi oraşe - capitală cu numere.",
          imagine: "examen2023-s1.png",
          subpuncte: [
            {
              A: {
                cerinta: "Precizati:",
                intrebari: [
                  {
                    intrebare: "numele statului marcat, pe hartă, cu litera G;",
                    raspunsuri: ["Bulgaria"],
                    tip: "completare",
                    punctaj: 2,
                  },
                  {
                    intrebare:
                      "numărul cu care este marcat, pe hartă, oraşul-capitală Roma.",
                    raspunsuri: ["1"],
                    tip: "completare",
                    punctaj: 2,
                  },
                ],
              },
              B: {
                cerinta:
                  "Scrieţi, pe foaia de examen, răspunsurile corecte care completează afirmaţiile de mai jos:",
                intrebari: [
                  {
                    intrebare:
                      "Orașul-capitală marcat, pe hartă, cu numărul 12 se numește...",
                    raspunsuri: ["Madrid"],
                    tip: "completare",
                    punctaj: 2,
                  },
                  {
                    intrebare:
                      " Statul marcat, pe hartă, cu litera E este situat în Munții ...",
                    raspunsuri: ["Pirinei"],
                    tip: "completare",
                    punctaj: 2,
                  },
                  {
                    intrebare:
                      " Statul marcat, pe hartă, cu litera C se numește...",
                    raspunsuri: ["Polonia"],
                    tip: "completare",
                    punctaj: 2,
                  },
                ],
              },
              C: {
                cerinta:
                  "Scrieţi, pe foaia de examen, litera corespunzătoare răspunsului corect pentru fiecare dintre afirmaţiile de mai jos:",
                intrebari: [
                  {
                    intrebare:
                      "Orașul-capitală marcat, pe hartă, cu numărul 13 se numește:",
                    optiuni: ["Belfast", "Dublin", "Londra", "Reykjavik"],
                    raspunsuri: ["Dublin"],
                    tip: "alegere",
                    punctaj: 2,
                  },
                  {
                    intrebare:
                      "În statul al cărui oraș-capitală este marcat, pe hartă, cu numărul 14 se află portul maritim:",
                    optiuni: ["Hamburg", "Le Hâvre", "Marsilia", "Pireu"],
                    raspunsuri: ["Hamburg"],
                    tip: "alegere",
                    punctaj: 2,
                  },
                  {
                    intrebare:
                      "Orașul marcat, pe hartă, cu numărul 9 este capitala statului numit:",
                    optiuni: ["Belarus", "Estonia", "Letonia", "Lituania"],
                    tip: "alegere",
                    raspunsuri: ["Lituania"],
                    punctaj: 2,
                  },
                  {
                    intrebare:
                      "SStatul marcat, pe hartă, cu litera B se numește:",
                    optiuni: ["Austria", "Cehia", "Elveția", "Slovenia"],
                    tip: "alegere",
                    raspunsuri: ["Elveția"],
                    punctaj: 2,
                  },
                  {
                    intrebare:
                      "Versantul estic al Alpilor (Munților) Scandinaviei se află în statul marcat, pe hartă, cu litera:",
                    optiuni: ["F", "H", "I", "J"],
                    tip: "alegere",
                    raspunsuri: ["I"],
                    punctaj: 2,
                  },
                ],
              },
              D: {
                cerinta:
                  "Prezentați trei deosebiri între clima statului marcat, pe hartă, cu litera A şi clima statului marcat pe hartă, cu litera D.",
                intrebari: [
                  {
                    intrebare: "",
                    optiuni: ["sbhsd", "dshduisyd87w8d", "3", "4", "5"],
                    raspunsuri: ["dshduisyd87w8d", "3"],
                    tip: "alegere-speciala",
                    punctaj: 2,
                  },
                ],
                //notele 1,2,3
              },
              E: {
                cerinta: "Alegeți variantele corecte: ",
                intrebari: [
                  {
                    intrebare:
                      "Prezentați un factor care a influențat dezvoltarea turismului în statul al cărui oraș-capitală este marcat, pe hartă, cu numărul 2.",
                    optiuni: ["aaa", "bbbb"],
                    raspunsuri: ["aaa"],
                    tip: "alegere-speciala",
                    punctaj: 2,
                  },
                  {
                    intrebare:
                      ". Explicați faptul că statul a cărui capitală este orașul marcat, pe hartă, cu numărul 11 obține energia electrică predominant din surse geotermale.",
                    optiuni: ["aaa", "bbbb"],
                    raspunsuri: ["aaa"],
                    tip: "alegere-speciala",
                    punctaj: 2,
                  },
                ],
              },
            },
          ],
        },
        //SUB II
        {
          cerinta:
            "Harta de mai sus se referă la subiectul II A – D. Pe hartă sunt marcate unităţi de relief cu litere, oraşe cu numere de la 1 la 6 şi râuri cu numere de la 7 la 12.",
          imagine: "examen2023-s2.png",
          subpuncte: [
            {
              A: {
                cerinta: "Precizati:",
                intrebari: [
                  {
                    intrebare:
                      "numele oraşului marcat, pe hartă, cu numărul 4;",
                    raspunsuri: ["Vaslui"],
                    tip: "completare",
                    punctaj: 2,
                  },
                  {
                    intrebare: "numele râului marcat, pe hartă, cu numărul 10.",
                    raspunsuri: ["Jiu"],
                    tip: "completare",
                    punctaj: 2,
                  },
                ],
              },
              B: {
                cerinta:
                  "Scrieţi, pe foaia de examen, răspunsurile corecte care completează afirmaţiile de mai jos:",
                intrebari: [
                  {
                    intrebare:
                      "Râul marcat, pe hartă, cu numărul 8 se numește ...",
                    raspunsuri: ["Argeș"],
                    tip: "completare",
                    punctaj: 2,
                  },
                  {
                    intrebare:
                      "Orașul marcat, pe hartă, cu numărul 3 se numește...",
                    raspunsuri: ["Zalău"], //ce vazut ce facem cu diacriticele
                    tip: "completare",
                    punctaj: 2,
                  },
                  {
                    intrebare:
                      "În subunitatea de relief marcată, pe hartă, cu litera A pătrund influențe climatice ...",
                    raspunsuri: [
                      "scandinavo baltice", //de vazut aici,
                      "continentale",
                      "ariditate",
                      "est europene",
                    ],
                    tip: "completare",
                    punctaj: 2,
                  },
                ],
              },
              C: {
                cerinta:
                  "Scrieţi, pe foaia de examen, litera corespunzătoare răspunsului corect pentru fiecare dintre afirmaţiile de mai jos:",
                intrebari: [
                  {
                    intrebare:
                      "Orașul marcat, pe hartă, cu numărul 6 se numește:",
                    optiuni: ["Buzău", "Pitești", "Ploiești", "Târgoviște"],
                    tip: "alegere",
                    raspunsuri: ["Ploiești"],
                    punctaj: 2,
                  },
                  {
                    intrebare:
                      " Râul marcat, pe hartă, cu numărul 12 străbate orașul:",
                    optiuni: [
                      "Cluj-Napoca",
                      "Bistrița",
                      "Sibiu",
                      "Târgu Mureș",
                    ],
                    tip: "alegere",
                    raspunsuri: ["Cluj-Napoca"],
                    punctaj: 2,
                  },
                  {
                    intrebare:
                      "Brațul Dunării marcat, pe hartă, cu numărul 11 se numește:",
                    optiuni: ["Borcea", "Chilia", "Sfântu Gheorghe", "Sulina"],
                    tip: "alegere",
                    raspunsuri: ["Sfântu Gheorghe"],
                    punctaj: 2,
                  },
                  {
                    intrebare:
                      "În unitatea de relief marcată, pe hartă, cu litera H se exploatează:",
                    optiuni: [
                      "aur și argint",
                      "gaz metan",
                      "minereu de fier",
                      " petrol și gaze naturale",
                    ],
                    tip: "alegere",
                    raspunsuri: ["petrol și gaze naturale"],
                    punctaj: 2,
                  },
                  {
                    intrebare:
                      "Orașul marcat, pe hartă, cu numărul 2 se numește:",
                    optiuni: [" Alba Iulia", "Deva", "Reșița", "Târgu Jiu "],
                    tip: "alegere",
                    raspunsuri: ["Deva"],
                    punctaj: 2,
                  },
                ],
              },
              D: {
                cerinta:
                  "Prezentați trei deosebiri între relieful unităţii marcate, pe hartă, cu litera B şi relieful unităţii marcate, pe hartă, cu litera E.",
                optiuni: ["sbhsd", "dshduisyd87w8d", "3", "4", "5"],
                //aici vreau sa bag si nota1, nota2, nota 3
                tip: "alegere-speciala",
                raspunsuri: ["sbhsd", "dshduisyd87w8d"],
                punctaj: 2,
                //2 puncter per desebire corecta
              },
              E: {
                cerinta: "",
                intrebari: [
                  {
                    intrebare:
                      ". Prezentați două cauze ale scăderii numerice a populației din România.",
                    optiuni: ["sbhsd", "dshduisyd87w8d", "3", "4", "5"],

                    tip: "alegere-speciala",
                    punctaj: 2,
                    raspunsuri: ["sbhsd", "dshduisyd87w8d"],
                    //2 puncter per desebire corecta
                  },
                  {
                    //daca aici nu am e ok sa las gol?
                    // intrebare:
                    //   "Precizați numele a două orașe cu peste 50.000 de locuitori situate în subunitatea de relief marcată, pe hartă, cu litera B.",
                    // optiuni: ["sbhsd", "dshduisyd87w8d", "3", "4", "5"],
                    // tip: "alegere-speciala",
                    // punctaj: 2,
                    // //2 puncter per desebire corecta
                  },
                ],
              },
            },
          ],
        },
        //SUB III
        {
          cerinta:
            "Reprezentarea grafică de mai jos se referă la subiectul III A-B și prezintă bilanțul (sporul) natural în Europa, în perioada 1950-2015. Sursa: population.un.org/wpp/DataQuery",
          imagine: "examen2024-s3.png",
          subpuncte: [
            {
              A: {
                cerinta: "Precizati:",
                intrebari: [
                  {
                    intrebare:
                      "valoarea maximă a bilanțului (sporului) natural, precum și perioada în care s-a înregistrat;",
                    //aici trebuie altfel sa dam interval
                    raspunsuri: [10.5, 10.8],
                    // ["se acceptă orice valoare cuprinsă între 10,5‰ și 10,8‰"],
                    tip: "interval",
                    punctaj: 2,
                  },
                  {
                    intrebare:
                      "valoarea minimă a bilanțului (sporului) natural, precum și perioada în care s-a înregistrat.",
                    raspunsuri: [-1.4, -1.6],
                    tip: "interval",
                    punctaj: 2,
                  },
                ],
              },
              B: {
                cerinta: "",
                intrebari: [
                  {
                    intrebare:
                      "Calculați diferența dintre valoarea maximă și valoarea minimă a bilanțului (sporului) natural din Europa.",
                    raspunsuri: [-1.4, -1.6],
                    tip: "interval",
                    punctaj: 2,
                  },
                  {
                    intrebare:
                      "Calculați diferența dintre valoarea bilanțului (sporului) natural în perioada 1965-1970 și valoarea bilanțului (sporului) natural în perioada 1985-1990.",
                    raspunsuri: [-1.4, -1.6],
                    tip: "interval",
                    punctaj: 2,
                  },
                  {
                    intrebare:
                      "Prezentați o cauză care explică scăderea bilanțului (sporului) natural din Europa, începând din anul 1990.",
                    optiuni: ["sbhsd", "dshduisyd87w8d", "3", "4", "5"],

                    tip: "alegere-speciala",
                    punctaj: 2,
                  },
                ],
              },
              C: {
                cerinta: "Pentru Ungaria, precizaţi numele a:",
                intrebari: [
                  {
                    intrebare: "trei state vecine;",
                    raspunsuri: ["Romania"],
                    tip: "alegere-speciala",
                    punctaj: 2, //2 per raspuns corect
                    optiuni: ["Romania", "Franta"],
                  },
                  {
                    intrebare: "două unități de relief;",
                    raspunsuri: ["Romania"],
                    tip: "alegere-speciala",
                    punctaj: 2, //2 per raspuns corect
                    optiuni: ["Romania", "Franta"],
                  },
                  {
                    intrebare: "3. două ape curgătoare; ",
                    raspunsuri: ["Romania"],
                    tip: "alegere-speciala",
                    punctaj: 2, //2 per raspuns corect
                    optiuni: ["Romania", "Franta"],
                  },
                  {
                    intrebare: "4. trei orașe",
                    raspunsuri: ["Romania"],
                    tip: "alegere-speciala",
                    punctaj: 2, //2 per raspuns corect
                    optiuni: ["Romania", "Franta"],
                  },
                ],
              },
              D: {
                cerinta:
                  "Tabelul de mai jos prezintă temperaturile medii lunare în ianuarie și în iulie, la stațiile meteorologice din orașele Paris (48,8º latitudine N și 2,5º longitudine E) și Kiev (50,4º latitudine N și 30,5º longitudine E).",
                //aici bag imaginea cu tabelul
                imagine: "examen2024-s3D.png",
                intrebari: [
                  {
                    intrebare:
                      "Calculați amplitudinea termică medie anuală în orașul Paris.",
                    raspunsuri: ["Paris: 15,1C"],
                    tip: "completare",
                    punctaj: 2,
                  },
                  {
                    intrebare:
                      "Calculați amplitudinea termică medie anuală în orașul Kiev.",
                    raspunsuri: ["Kiev: 24,5C"],
                    tip: "completare",
                    punctaj: 2,
                  },
                  {
                    intrebare:
                      " Prezentați o cauză care explică diferența dintre valoarea amplitudinii termice anuale din cele două orașe, deși sunt situate la latitudini apropiate.",
                    //de pus o lista de raspunsuri
                    //sau de pus cu alegere
                    raspunsuri: ["aa"],
                    optiuni: ["aa", "bbb"],
                    tip: "alegere-speciala",
                    punctaj: 2,
                  },
                ],
              },
              E: {
                cerinta: "Prezentați o cauză care explică:",
                intrebari: [
                  {
                    intrebare:
                      "suprafețele mari cultivate cu cereale în Câmpia de Vest;",
                    raspunsuri: ["aa"],
                    optiuni: ["aa", "bbb"],
                    tip: "alegere-speciala",
                    punctaj: 2,
                  },
                  {
                    intrebare:
                      "lipsa viețuitoarelor în stratul de adâncime al apelor Mării Negre",
                    raspunsuri: ["aa"],
                    optiuni: ["aa", "bbb"],
                    tip: "alegere-speciala",
                    punctaj: 2,
                  },
                ],
              },
            },
          ],
        },
      ],
    },
    { tip: "simulare", nume: "Simulare 2024", puncte: [] },
    { tip: "test", nume: "Test" },
  ],
};

module.exports = an2023;
